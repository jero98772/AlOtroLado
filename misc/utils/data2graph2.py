...
#exit()
sum(data["harassmentRisk"])/len(data["harassmentRisk"])
def getmMeanPond():
  n1=0
  #n2=0
  for i in range(len(data))
    n1+=data["length"]*data["harassmentRisk"]
  mean=n1/sum(data["length"])
  return mean
#w distancia,x riesgo

for i in range(len(data)): 
  origin=(data["origin"][i][1:-1].split(","))
  destination=(data["destination"][i][1:-1].split(","))
  try:
    dataclear+='{"name":"'+data["name"][i]+'","path": ['+"["+origin[0]+","+origin[1]+"]"+",["+destination[0]+","+destination[1]+']],"node":['+origin[0]+","+origin[1]+'],"harassmentRisk":'+str(data["harassmentRisk"][i]).replace("nan","0")+',"length":'+str(data["length"][i])+',"weigth":'+str(data["length"][i])+'},'
  except:
    print(data["name"][i])
